...
def detectHDMIID(sinks_dict):
    for sink in sinks_dict:
        for k, v in sink.items():
            # print(str(k) + ': ' + v)
            if str(v).startswith('Baffin'):
                hdmi_id = k
                return hdmi_id
...
hdmi_id = detectHDMIID(sinks_dict)

# print(hdmi_id)

subprocess.run('wpctl set-default ' + str(hdmi_id), shell=True, encoding='utf-8')
